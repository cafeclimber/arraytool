...
def at_export(data=np.ones((0, 0)), data_ID=False, fmt='%.4e', mode='a'):
    ...
    master = ti.Tk()
    master.withdraw()  # hiding tkinter window
    file_path = filedialog.asksaveasfile(mode,
                                         title="Save file",
                                         filetypes=[("txt file", ".csv"),
                                                    ("All files", ".*")])
    master.quit()
...
def ip_format(a,
    ...
    if (plot):
        ...
        if ((abs(A.imag) > 1e-10).sum()):
            A_plt = abs(
                A)  # if A.imag are significant, then '|A|' will be plotted
        else:
        ...
        if (M == 1):  # i.e, linear array is along the y-direction
            ...
            if (show):
                plt.title(r'$\mathrm{Array}\ \mathrm{Excitation}$',
                          fontsize=18)
                plt.show()
            ...
            if (mayavi_app
                ):  # this option opens the 3D plot in MayaVi Application
...
def AF_zeros(a, M, R, dist_type, nbar=False, alpha=0):
    ...
    if (dist_type == "Dolph-Chebyshev"):  # Dolph zeros
        ...
        U0 = (2 / (a * k)) * np.arccos(
            (np.cos(np.pi * (2 * n - 1) / (2 * M - 2))) / c)
    elif (dist_type == "Riblet"):  # Riblet zeros
        c1 = np.cosh(np.arccosh(R) / m)
        c = np.sqrt((1 + c1) / (2 + (c1 - 1) * np.cos(k * a / 2)**2))
        alph = c * np.cos(k * a / 2)
        xi = (1 / c) * np.sqrt(((1 + alph**2) / 2) + (
            (1 - alph**2) / 2) * np.cos(((2 * n - 1) * np.pi) / (2 * m)))
        U0 = (2 / (a * k)) * np.arccos(xi)
    elif (dist_type == "Duhamel-b"
          ):  # Duhamel bi-directional end-fire array zeros
        if (a < 0.5):
        ...
        U0 = (2 / (a * k)) * np.arcsin(
            (np.cos(np.pi * (2 * n - 1) / (2 * M - 2))) / c)
    elif (dist_type == "Duhamel-u"
          ):  # Duhamel uni-directional end-fire array zeros
        Lamb = np.cosh(np.arccosh(R) / (M - 1))
        xi = (2 /
              a) * (0.5 * np.pi -
                    (np.arctan(np.tan(k * a / 2) * ((Lamb + 1) / (Lamb - 1)))))
        c = 1 / (np.sin((xi - k) * a / 2))
        ...
        U0 = "Yet to be done"
          ...
          ):  # McNamara-Zolotarev difference-pattern zeros
    ...
    if (nbar):  # Taylor's Dilation procedure
        if ((dist_type == "Dolph-Chebyshev") or (dist_type == "Riblet")
                or (dist_type == "McNamara-s")):
...
def pattern_u(array_ip,
    ...
    if (z_flag):
        ...
        if (factor == "AF"):
            ...
            P_inc = ((abs(A))**2).sum()
            GF = AF / np.sqrt(
                P_inc)  # Converting the AF to GF ... AKB's definition
            F = GF
            n1 = ""
        ...
        if (plot_type):
            if (plot_type == "rect"):  # rectangular plot
                plt.plot(u,
                         ...
                         alpha=alpha)
                if (lattice
                    ):  # highlighting the visible-space and unit-lattice
                ...
                plt.grid(True)
                plt.xlabel(
                    r'$u,\ \mathrm{where}\ u=\sin \theta\ \mathrm{in}\ \mathrm{the}\ \mathrm{visible-space}$',
                    fontsize=16)
...
def pattern_t(array_ip,
    ...
    :rtype:                [phi,F] ... F can be AF/GF/NF/NF0 ...
                           and can be either in linear or in dB scale ...
    """
    tht, phi, F, AF = pattern_tp(array_ip,
                                 tht_scan=tht_scan,
                                 phi_scan=phi_scan,
                                 tht_min=tht,
                                 tht_max=tht,
    ...
    if (scale == "linear"):
        F_plt = abs(
            F)  # pattern_tp fives linear values ... some of them can be -ve
    elif (scale == "dB"):
        F_plt = cutoff(F,
                       dB_limit)  # pattern_tp gives F in dB without any cutoff
