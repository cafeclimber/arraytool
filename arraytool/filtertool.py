...
def ft_import(dtype='complex'):
    ...
    file_path = tkdlg.askopenfilename(title="Open file",
...
def Chebyshev_gen(N, poles):
    ...
    if (len(poles) == 0):
        ...
        for j in range(1, len(poles)):
            if (np.isinf(poles[j])):
                ...
                U = x * tmp1 - tmp1 / poles[j] + sp.sqrt(x**2 - 1) * np.sqrt(
                    1 - 1 / poles[j]**2) * tmp2
                V = x * tmp2 - tmp2 / poles[j] + sp.sqrt(x**2 - 1) * np.sqrt(
                    1 - 1 / poles[j]**2) * tmp1
                tmp1 = sp.simplify(U)
                tmp2 = sp.simplify(V)
        U = tmp1
    ...
    Num = np.reshape(
        Num,
        (len(Num), -1)) / norm  # so that abs(polynomial) value becomes 1 at +1
    return Num, Den
...
def plot_delay(roots_E, w_min=-2, w_max=2, w_num=500, plot=True):
    ...
    for i in range(len(w)):
        ...
        for j in range(len(roots_E)):
            tmp += -roots_E[j].real / (roots_E[j].real**2 +
                                       (w[i] - roots_E[j].imag)**2)
        tau[i] = tmp
    if (plot):
        plt.plot(w, tau)
        plt.axis('tight')
        plt.grid(True)
        plt.xlabel(r'$\Omega\ \mathrm{(rad/s)}$', fontsize=14)
        plt.ylabel(r'$\mathrm{Group}\ \mathrm{delay} \mathrm{\ (s)}$',
                   fontsize=14)
        plt.show()
    return w, tau
    ...
    if (p == q):
        if (p == i):
            ...
            tht = np.arctan(
                (-M[i - 1, j - 1] +
                 np.sqrt(M[i - 1, j - 1]**2 -
                         M[i - 1, i - 1] * M[j - 1, j - 1]) / M[j - 1, j - 1]))
        else:
            print('p!=i')
            tht = np.arctan((M[i - 1, j - 1] + np.sqrt(
                (M[i - 1, j - 1]**2 - M[i - 1, i - 1] * M[j - 1, j - 1]))) /
                            M[i - 1, i - 1])
    else:
        print(p != i)
        if (p == i and q == j):
            tht = 0.5 * np.arctan(2 * M[i - 1, j - 1] /
                                  (M[j - 1, j - 1] - M[i - 1, i - 1]))
        elif (p == j and q == i):
            k = p
            tht = 0.5 * np.arctan(
                (2 * M[k - 1, i - 1]) / (M[k - 1, k - 1] - M[j - 1, j - 1]))
