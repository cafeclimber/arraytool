    ...
    if (plot_type):
        ...
        if (plot_type == "2D"):  # plot 2D plot in Matplotlib
            plt.plot(position_beta,
                     ...
                     alpha=alpha)
            if (stem):
                plt.stem(position_beta,
                         A_plt,
                         linefmt=stemline,
                         markerfmt=stemmarker)
            plt.axis('tight')
            plt.grid(True)
            ...
            s1 = mlab.quiver3d(position_x, position_y, position_z, position_z,
                               position_z, A_plt)  # stem3D representation
            ranges1 = [
                position_x.min(),
                position_x.max(),
                position_y.min(),
...
def FS(fun_str_re,
    ...
    for m in m_index:
        fun_re = lambda x: (eval(fun_str_re)) * np.cos(m * w0 * x) + (eval(
            fun_str_im)) * np.sin(m * w0 * x)
        fun_img = lambda x: -(eval(fun_str_re)) * np.sin(m * w0 * x) + (eval(
            fun_str_im)) * np.cos(m * w0 * x)
...
def dist_c_az(P,
    ...
    if (plot_modes):
        ...
        plt.grid(True)
        plt.title(
            r'$\mathrm{Far\ \&\  Near-field\ \ Modes \ (f_m\ \& \ a_m)}$')
